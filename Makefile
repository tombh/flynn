...
test-unit-deps: toolchain
	@$(GIT_DEV) $(GO_ENV) tup appliance/etcd discoverd host/cli/root_keys.go installer/bindata.go dashboard/bindata.go

test-unit: test-unit-deps
	@$(GO_ENV) PATH=${PWD}/appliance/etcd/bin:${PWD}/discoverd/bin:${PATH} util/_toolchain/go/bin/go test -x -race -cover ./...
